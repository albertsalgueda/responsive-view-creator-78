...
const Services2 = ({
  ...
  columnsText = "Product Strategy & Management<p>User Research & Testing<p>Website Design & Development<p>Mobile App Design & Development<p>Motion & Interaction Design<p>Design Systems<p>Concept Proofing & Prototyping"
}: Services2Props) => {
      ...
      <div className="max-w-7xl w-full mx-auto h-full p-0 flex flex-col">
        {isMobile ? <div className="grid grid-cols-3 gap-4 px-6 h-full py-0">
            ...
            lineHeight: 1.1
                ...
                {columnsText.split('<p>').map((item, index) => <p key={index} className="mb-2">{item}</p>)}
              </div>
            </div>
            ...
            lineHeight: 1.1
                ...
                {columnsText.split('<p>').map((item, index) => <p key={index} className="mb-3">{item}</p>)}
              </div>
            </div>
          </div>}
      </div>
