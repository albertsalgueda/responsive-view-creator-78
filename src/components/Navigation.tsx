...
const Navigation = ({
  ...
  const MobileMenu = () => <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent className="h-[100vh] bg-opacity-100 border-none py-8 flex flex-col justify-between" style={{ 
        ...
        <div className="flex flex-col items-start pt-16 px-6">
          ...
          <div className="flex flex-col gap-5 items-start w-full pb-[20px]">
            {links.map((link, index) => <a key={index} href={link.href} style={{
              ...
              color: textColor,
              transition: transition
            }} onClick={() => setOpen(false)} className="text-3xl font-extrabold font-barlow italic font-weight-800 hover:opacity-80 transition-all">
                {link.text}
              </a>)}
          </div>
        </div>
